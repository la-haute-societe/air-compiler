...
var that = {
	...
	compile: function (pConfigNode)
	{
		...
		if (configNode.debug != null && configNode.debug != "")
		{
			...
			if (debuggingIP != null && debuggingIP != "")
			{
				...
				cli.notice("Selected IP for debugging from config : " + _ipForDebugging);
			}
			else
			{
				// Selected IPs
	...
	subCompile: function (pConfigNode, pTargetNode, pTargetName)
	{
		...
		if (pTargetName == "ios")
		{
            ...
            // Legacy AOT compiler
            if ("newCompiler" in pTargetNode)
            {
                buildParameters.push("-useLegacyAOT " + (pTargetNode.newCompiler ? "no" : "yes"));
            }

			// Enable sampling
			if ("sampling" in pTargetNode && pTargetNode.sampling != false)
			{
				buildParameters.push("-sampler");
			}

			// Hide ane libs
			if ("hideAneLib" in pTargetNode && pTargetNode.hideAneLib != false)
			{
	...
	listDevices: function (pPlatform, pCallback)
	{
		...
		var androidIDPattern = /([0-9A-Z]{12})/g;
